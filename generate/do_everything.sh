...
$XRAY_BIT2FASM $BATCHDIR/vivado/bs/$DESIGN.bit > $BATCHDIR/fasm/$DESIGN.fasm
bash utils/extract.sh $BATCHDIR/vivado/bs/$DESIGN.bit $BATCHDIR/mem/$DESIGN.mem
python3 utils/addr_interpret.py -fasm $BATCHDIR/fasm/$DESIGN.fasm -outfile $BATCHDIR/analysis/addrs/${DESIGN}.addrs
python3 utils/simplify_fasm.py -fasm $BATCHDIR/fasm/$DESIGN.fasm -outfile $BATCHDIR/analysis/shortfasm/$DESIGN.ifasm
